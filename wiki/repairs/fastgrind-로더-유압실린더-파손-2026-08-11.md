...
## 2026-09-14 — 정식수리 착수 (F 장비)

- **부품 입고 완료** 후 **2026-09-14(월) F(FastGrind) 1대 실린더 장착 진행 중** [한경준님 확인].
- 2026-08-14 ANCA 정식 발주 이후 약 **1개월** — 발주 시 예상 리드타임(약 1개월)과 부합.
- 2026-08-13 에이원 임시조치(파이프 조정·1개 삽입·케이블타이 고정)로 가동을 이어온 구간은 **약 1개월**이며, 당시 추정 내구성 2~3개월 이내에 정식수리로 전환됐다.
- ⚠️ **장착일 가동 중단분**은 2026-09-14 일일보고 실적에 반영된다 — 무실적·저실적을 자동화 오류로 오판하지 않는다.
- 잔여 확인: 정확한 착수·완료 시각, 장착 후 그리퍼 파지정밀도·파레트 안착위치 재점검 여부, 시운전 결과.
- 🔴 **2026-09-14 당일 생산 실적 0 — 수리로 인한 가동 중단이다** [한경준님 확인]. 일일보고·캘린더에 `FAST 0 / GX7 0` 으로 찍히나 **무실적(수주 없음)이 아니라 설비 정지**다.
  ⚠️ **생산량 통계에서 이 날을 평상일로 세지 않는다** — 「143 조업일 실측」류 분석(로더 유무 대비 생산량 등)에 넣으면 평균이 왜곡된다. GX7 장착일(09-21 주 예정)도 같게 취급한다.
  ⚠️ `daily-report-check`·`scheduler-diagnose` 로 점검할 때 이 날의 0을 **자동화 오류로 오판하지 않는다.**
