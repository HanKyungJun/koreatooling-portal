...
> 🔴 **2026-09-09 정정 — 이 절은 보유 휠에 적용되지 않는다** [실측 검증]. 기존 서술은 삭제하지 않고 성격만 한정한다.
>
> **EN 12413:2019(= 12413 계열)의 적용범위가 초지립 제품을 명시적으로 제외**한다 — *"does not apply to
> super abrasive products and coated abrasive products"* [STD-EN12413, BSI 제품 페이지]. 보유 휠 5종은
> 전부 **레진 본드 다이아몬드 = 초지립**이므로 **대상이 아니다.**
> → 적용 표준은 **[STD-EN13236] EN 13236:2019 (Safety requirements for superabrasive products)** 다.
> **oSa 표기 요건**이 이 표준을 근거로 **최대사용주속도(m/s) 제품 표기**를 요구한다(다이아몬드 와이어만 예외)
> [STD-OSA-SUPER-MARK].
>
> ⚠️ **위 「Resin Bond 일반 Vmax 30~60 m/s」의 출처가 불분명해졌다.** 12413 계열이 초지립을 다루지 않으므로
> 이 수치는 이 표준에서 나올 수 없다. **[신뢰도: 추정값]** 으로 재분류한다 — 근거를 다시 붙이거나 삭제할지는
> EN 13236 본문 확보 후 판단한다(본문 유료, 미확보).
>
> ⚠️ **같은 절의 「실제 운영 Vc는 Vmax의 70-80% 이하 권장」도 출처가 없다** — 위 재분류에서 빠져 있어 함께 **[신뢰도: 추정값]** 으로 재분류한다(기존 서술 보존). [검수 추가 2026-09-23]
>
> ⚠️ **번호 확인 필요**: sources.md 의 iso.org 링크가 **404** 이고 검색으로도 ISO 판이 확인되지 않는다.
> 실재가 확인되는 것은 **EN 12413:2019(CEN)** 다. ISO 판 존재 여부는 확인 필요 — 「없다」고 단정하지 않는다.
>
> 🟢 **JIS 계열에는 초지립 Vmax 규정이 아예 없다** — JIS R 6241:2018 「最高使用周速度」의 적용범위가
> **보통 연삭숫돌 전용**이며 다이아·CBN 미포함 [STD-JISR6241, 본문 직접 판독]. 오르가닉 1호 평형 45 m/s /
> 중연삭 40 m/s, 색帯 청 ≤45 · 황 45~60 · 적 >60 m/s 도 전부 보통 숫돌용이다.
...
| **절대 한계 (Vmax)** | ~~휠 측면 라벨 확인 (ISO 12413:2019 의무)~~ → **[STD-EN13236] EN 13236:2019** (초지립 표준, 2026-09-09 정정) | ~~일반 Resin: 30-45 m/s~~ **[추정값 — 출처 불명, 2026-09-09 재분류]**; 휠별 상이 |
...
8. `raw/notes/archive/휠RPM_권장도표_v3_정밀화.xlsx` (사용자 작성, 2026-05-15)
