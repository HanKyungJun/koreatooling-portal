"""
⛔ 폐기(DEPRECATED) — 2026-09-03
====================================================================
이 스크립트는 **erp/daily_dlv_alert.py (납기 알림 자동화 v2.0)** 으로
대체되었습니다. 실행하지 마세요.

폐기 사유
  - v2.0 이 "SMTP 제거, Gmail API 사용" 으로 발송 방식을 교체했습니다.
    이 파일은 SMTP + Gmail 앱 비밀번호를 쓰는 v1 입니다.
  - 그 앱 비밀번호는 공개 저장소 히스토리(eaf8e6f)에 평문 노출되어
    2026-09-03 에 폐기되었습니다. 따라서 이 파일은 실행해도 인증에
    실패합니다.
  - Windows 작업 스케줄러·배치 어디에도 등록된 적이 없습니다.
    (독스트링의 "매일 08:30 권장" 은 계획이었을 뿐 실행된 적 없음)
  - 2026-06-11 이후 수정 이력 없음.

현재 운영 중인 납기 알림
  erp/daily_dlv_alert.py  (Gmail API / OAuth, 매일 09:00)
    산출물: outputs/dlv_alerts/납기알림_YYYYMMDD.xlsx
    인증  : erp/token_gmail.json + client_secret_gmail.json

기록: wiki/_handoff/decisions.md 2026-09-03 (3)
※ 원칙에 따라 삭제하지 않고 보존합니다. 참고용으로만 두세요.
"""
