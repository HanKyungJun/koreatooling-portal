...
# ── 2026-09-14 추가: 「대기」 신호 ─────────────────────────────────────────────
#   🔴 는 이 위키에서 「긴급」이 아니라 「확정하지 않았다」는 뜻으로 쓰인다.
#   그래서 A-긴급이 매번 ①의도적으로 열어 둔 항목(추정 금지 원칙) ②외부 회신·방문
#   대기 항목으로 채워져, 첫 블록에 「지금 손댈 수 있는 것」이 거의 없었다(09-14 실측 10건 중 2건).
#   ⚠️ ✅ 기준은 쓰지 않는다 — 2026-08-28 에 폐기된 규칙이고 오판이 많다.
#      사람이 의도를 적어 둔 표현만 본다.
WAIT = re.compile(r'추정하지\s*않|추정\s*금지|열어\s*둔다|열어\s*둔|방침|보류|'
                  r'회신\s*대기|회신을\s*기다|문의하기로|대기\s*중|결과가\s*나오면')
...
def collect():
    ...
    for r in rows:
        ...
        r[10] = ('D-중복' if r[9] == '중복' else
                 'C-해소추정' if r[5] else
                 'A-대기' if (r[4] and r[11]) else
                 'A-긴급' if r[4] else
                 'B-장기미해결' if el > 365 else
                 'B-경과90일+' if el > 90 else 'C-일반')
    order = {'A-긴급': 0, 'A-대기': 1, 'B-장기미해결': 2, 'B-경과90일+': 3,
             'C-일반': 4, 'C-해소추정': 5, 'D-중복': 6}
...
def build(rows):
    ...
    info = [
        ...
        ('', ''),
        ('■ 우선순위 정의', ''),
        ('A-긴급', f'🔴 표기가 있고 **대기 신호가 없는** 항목 — 지금 손댈 수 있는 것 ({cnt.get("A-긴급",0)}건)'),
        ('A-대기', f'🔴 표기가 있으나 「추정하지 않는다·열어 둔다·방침·보류·회신·문의하기로」 '
                   f'등 대기 표현이 함께 있는 항목 — 일부러 열어 뒀거나 외부 대기 ({cnt.get("A-대기",0)}건). '
                   f'⚠️ 2026-09-14 신설. 이전 판에서는 전부 A-긴급으로 세었으므로 '
                   f'A-긴급 건수를 09-07판 이전과 직접 비교하지 말 것 — 분류만 바뀐 것이다.'),
        ...
        ('', ''),
        ('■ 사용법', ''),
        ('1', 'A-긴급(지금 할 수 있는 것) → A-대기(무엇을 기다리는지·얼마나 됐는지 확인) → B-장기미해결 순으로 훑는다. A-대기의 경과일이 길면 「기다린다」는 방침 자체를 재검토한다.'),
    ...
    col = {'A-긴급': BAD, 'A-대기': WARN, 'B-장기미해결': WARN, 'B-경과90일+': WARN,
