...
# ── 원본 데이터 ──────────────────────────────────────────
src = 'raw/출하현황/출하현황_20260417082841.xls'
df = pd.read_excel(src, sheet_name='출하현황', header=0)
...
tc = ws['A1']
tc.value     = "2025년 출하현황 — 납품처별 월별 출고량  (단위: 개)"
...
for ri, row in pivot.iterrows():
    ...
    c = ws.cell(row=er, column=1, value=rank);           data_style(c, bg=bg)
    c = ws.cell(row=er, column=2, value=row['납품처명']); data_style(c, center=False, bg=bg)
...
for m in range(1, 13):
    ...
    tc = ws_m['A1']
    tc.value     = f"2025년 {m}월 출하현황 — 납품처별 출고량  (단위: 개)"
    tc.font      = Font(name=FONT, bold=True, size=13, color=HDR_FG)
    tc.fill      = PatternFill("solid", fgColor=HDR_BG)
    tc.alignment = Alignment(horizontal='center', vertical='center')
    ...
    for ri, row in mdata.iterrows():
        ...
        c = ws_m.cell(row=er, column=1, value=rank)
        data_style(c, bg=bg)

        c = ws_m.cell(row=er, column=2, value=row['납품처명'])
        data_style(c, center=False, bg=bg)
    ...
    ws_m.cell(row=tot_row_m, column=5, value='')
...
out = 'wiki/comparisons/출하현황_납품처별_월별분석_2025.xlsx'
wb.save(out)
print(f"저장 완료: {out}")
